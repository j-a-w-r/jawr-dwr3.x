...
import java.util.*;
 ...
 /**
 * A simple DWR 3 ResourceGenerator for JAWR inspired by the one from Jordi Hernandez Selles
...
public class DWR3BeanGenerator extends AbstractJavascriptGenerator implements ResourceGenerator {
	...
	public Reader createResource(GeneratorContext context) {
        ...
        try {
            if (dwrLibraries.containsKey(context.getPath())) {
               // Static script
                data = getStaticScript(dwrLibraries.get(context.getPath()), context);
            ...
            throw new RuntimeException("An IO error occured while getting content from DWR", e);
        }
     ...
     * @param path The Script URL (i.e.: /engine.js)
     * @param context The GeneratorContext
     * @return The script as a String
     * @throws IOException
     */
    private String getStaticScript(final String path, final GeneratorContext context) throws IOException {
        CachingHandler cachingHandler = (CachingHandler) getContainer(context).getBean(path);
        return cachingHandler.generateCachableContent(context.getServletContext().getContextPath(), context.getConfig().getDwrMapping(), "");
    }
    /**
    ...
    private Container getContainer(final GeneratorContext context) {
        List<Container> containers = StartupUtil.getAllPublishedContainers(context.getServletContext());
        // TODO: here we assume there is only one DWR Servlet / Container ... we should find a way to math the container with the DWR mapping config
     ...
     * @throws IOException
     */
    private String getInterfaceScript(String scriptName, final GeneratorContext context) throws IOException {
        ...
        if (ctManager.getCreator(scriptName, false) != null) {
            InterfaceHandler handler = (InterfaceHandler) ContainerUtil.getHandlerForUrlProperty(container, INTERFACE_HANDLER_URL);
            return handler.generateInterfaceScript(context.getServletContext().getContextPath(), context.getConfig().getDwrMapping(), scriptName);
        } else
    ...
    private String getAllPublishedInterfaces(final GeneratorContext context) throws IOException {
        ...
        if (null != ctManager) {
           ...
           if (ctManager instanceof DefaultCreatorManager) {
                ...
                creatorManager.setDebug(true); // DWR will throw a ScurityException if not in debug mode ...
            ...
            for (String name : creators) {
                ...
                InterfaceHandler handler = (InterfaceHandler) ContainerUtil.getHandlerForUrlProperty(container, INTERFACE_HANDLER_URL);
                sb.append(handler.generateInterfaceScript(context.getServletContext().getContextPath(), context.getConfig().getDwrMapping(), name));
            }
        }
